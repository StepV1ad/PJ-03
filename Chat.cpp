...
void Chat::addCommonMessage()
{
	...
	std::cout << std::endl;
	commonChat_.push_back(Message{ currentUser_->getUserName(), "all", text });
}
...
void Chat::addUserMessage()
{
	...
	if (!checkName(to))
	{
		...
		messages_.push_back(Message{currentUser_->getUserName(), to, text });
}
...
void Chat::changeUser()
{
	...
	users_.insert(users_.begin() + currentUser_->getID(), User(currentUser_->getUserLogin(), currentUser_->getUserPassword(), currentUser_->getUserName(), currentUser_->getID()));
}
